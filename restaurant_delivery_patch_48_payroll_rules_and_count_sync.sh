...
echo "== patch_48_payroll_rules_and_count_sync =="
echo "Purpose: one manager-approved payroll rule engine, synced counts, and restored quick fixes."
echo "This patch REPLACES the existing payroll service and final payroll view block. It does not append duplicate view functions."
...
backup_dir="patch_backups_48_${stamp}"
mkdir -p "$backup_dir"
cp -f core/services/payroll.py "$backup_dir/payroll.py.before_patch48" 2>/dev/null || true
cp -f core/views.py "$backup_dir/views.py.before_patch48"
cp -f templates/payroll_problems.html "$backup_dir/payroll_problems.html.before_patch48" 2>/dev/null || true
...
<head>
    ...
    <style>
        ...
        .review { color: #92400e; font-weight: bold; }
        .ok { color: #1a7f37; font-weight: bold; }
        .button, button { display: inline-block; padding: 9px 12px; background: #2563eb; color: white; text-decoration: none; border: 0; border-radius: 8px; font-weight: bold; margin: 2px; cursor:pointer; }
        ...
        .info { background:#eff6ff; border-left:4px solid #2563eb; padding:10px; margin:12px 0; }
        ...
        .actual-form { display:inline-flex; gap:6px; align-items:center; }
...
<p>Fix only the clock records that would stop Sage payroll from being safe. Review items do not block export.</p>
...
{% if problem_count == 0 %}
    <div class="ready"><strong>Payroll ready.</strong> No clocking blockers found for this week.</div>
{% else %}
    <div class="note"><strong>Payroll not ready: {{ problem_count }} blocker(s) must be fixed before Sage export.</strong></div>
{% endif %}
...
<table>
    ...
    <tr>
        ...
        <td>{% include "payroll_quick_actions.html" with row=row week_start=week_start %}</td>
    </tr>
    {% empty %}
    <tr><td colspan="7" class="ok">No payroll blockers found.</td></tr>
    {% endfor %}
</table>

<h2>Manager Review</h2>
<div class="info">These items are worth checking, but they do not block Sage export.</div>
<table>
    <tr><th>Date</th><th>Employee</th><th>Roster</th><th>Status</th><th>Worked</th><th>Review</th><th>Action</th></tr>
    ...
    <tr>
        ...
        <td>{{ row.status }}</td>
        <td>{{ row.worked_hours }}h</td>
        <td class="review">{{ row.problem }}</td>
        <td>{% include "payroll_quick_actions.html" with row=row week_start=week_start %}</td>
    </tr>
    {% empty %}
    <tr><td colspan="7" class="ok">No manager review items.</td></tr>
    {% endfor %}
</table>

<p class="help">Examples: missed clock-out is a payroll blocker. Worked but not rostered is a manager review item and can be approved.</p>
...
cat > templates/payroll_quick_actions.html <<'HTML'
<div class="quick-row">
{% for action in row.quick_actions %}
    {% if action.mode == 'advanced' %}
        <a class="advanced" href="/manager/fix-day/?employee_number={{ row.employee_number }}&event_date={{ row.date|date:'Y-m-d' }}&week_start={{ week_start|date:'Y-m-d' }}">{{ action.label }}</a>
    {% elif action.mode == 'enter_actual_start' or action.mode == 'enter_actual_finish' %}
    ...
    {% else %}
        <form method="post" style="display:inline;">
            ...
            <input type="hidden" name="event_date" value="{{ row.date|date:'Y-m-d' }}">
            <input type="hidden" name="week_start" value="{{ week_start|date:'Y-m-d' }}">
            <button class="button {% if action.mode == 'approve_unrostered_shift' %}approve{% else %}fix{% endif %} smallfix" type="submit">{{ action.label }}</button>
        </form>
    {% endif %}
{% endfor %}
</div>
...
if marker not in s:
    raise SystemExit("ERROR: Could not find Patch 45 payroll block in core/views.py. Refusing to append duplicate views.")
...
python3 -m py_compile core/services/payroll.py core/views.py

echo "Running Django check..."
python3 manage.py check

echo
echo "Patch 48 complete."
echo
echo "What changed:"
echo "  - Payroll Blockers and Manager Review are now separate."
echo "  - Home/dashboard, Weekly Payroll, Payroll Issues and Sage export use the same blocker count."
echo "  - Quick fixes are visible again: use roster start/finish, use rostered shift, enter actual times, approve unrostered shift."
echo "  - Existing final payroll view block was replaced; no duplicate payroll view functions were appended."
echo
echo "Next commands:"
echo "  git status"
echo "  git add core/services/payroll.py core/views.py templates/payroll_problems.html templates/payroll_quick_actions.html"
echo "  git commit -m \"Patch 48 sync payroll rules and restore manager fixes\""
echo "  git push"
