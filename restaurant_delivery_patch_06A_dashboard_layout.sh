...
echo "=== Restaurant Clocking Patch 06A: Manager Dashboard Layout ==="
echo "Safe dashboard-only patch:"
echo "  - New top cards: Working, On Break, Late, Not Arrived, Payroll Issues, Payroll Ready"
echo "  - Staff Currently Working moved to top"
echo "  - Urgent + Operational merged into Needs Attention"
echo "  - Removes Clocked Out as headline card"
echo
...
mkdir -p patch_backups_06a
cp -f core/views.py patch_backups_06a/views.py.bak
cp -f templates/manager_today.html patch_backups_06a/manager_today.html.bak
...
template_path = Path("templates/manager_today.html")

views = views_path.read_text()

matches = list(re.finditer(r"def manager_today_dashboard\(request\):", views))
if not matches:
    raise SystemExit("Could not find manager_today_dashboard in core/views.py")
...
if '"needs_attention_rows": needs_attention_rows' not in func:
    func = func.replace(
...
<div class="container">
    <div class="header">
        ...
        <p class="muted">Quick view for the manager: who is here, who is on break, who is late, and whether payroll needs attention.</p>
    ...
    <div class="cards">
        ...
        <div class="card"><div class="card-title">⏰ Late</div><div class="number {% if late_count > 0 %}red{% endif %}">{{ late_count }}</div></div>
        ...
        <div class="card"><div class="card-title">⚠ Payroll Issues</div><div class="number {% if payroll_issues_count > 0 %}red{% else %}green{% endif %}">{{ payroll_issues_count }}</div></div>
        <div class="card"><div class="card-title">✅ Payroll Ready</div><div class="number {% if payroll_ready < 80 %}red{% elif payroll_ready < 100 %}orange{% else %}green{% endif %}">{{ payroll_ready }}%</div></div>
    </div>

    <div class="section">
        <h2>Staff Currently Working</h2>
        <p class="muted">This is the live floor view: who is in the building and whether anyone is on break.</p>
        <table>
            <tr><th>Employee</th><th>Status</th><th>Roster</th><th>First In</th><th>Worked</th><th>Break</th></tr>
    ...
    <div class="section">
        <h2>Today Roster Status</h2>
        <p class="muted">A quick check of the whole day: who was expected and what their current status is.</p>
        ...
        <table>
            <tr><th>No</th><th>Employee</th><th>Roster</th><th>First In</th><th>Last Out</th><th>Status</th><th>Issue</th></tr>
            {% for row in rows %}
                {% if row.rostered %}
                <tr>
                ...
                {% endif %}
            {% empty %}
            <tr><td colspan="7">No rostered staff today.</td></tr>
            {% endfor %}
        </table>
    </div>
    ...
    <div class="section">
        ...
        <p class="muted">Everything the manager may need to look at today. Payroll issues are shown in red; operational notes are shown in amber.</p>
        <table>
            <tr><th>Employee</th><th>Type</th><th>Roster</th><th>Status</th><th>Issue</th><th>Worked</th><th>Break</th></tr>
            ...
            {% empty %}
            <tr><td colspan="7" class="ok">No issues need attention.</td></tr>
            {% endfor %}
        </table>
    </div>
    ...
    <div class="section">
        ...
        <p><strong>Payroll Ready:</strong>
            <span class="{% if payroll_ready < 80 %}urgent{% elif payroll_ready < 100 %}operational{% else %}ok{% endif %}">{{ payroll_ready }}%</span>
        </p>
        <p><strong>Payroll issues remaining:</strong> {{ payroll_issues_count }}</p>
        <p class="muted">Resolve payroll issues before exporting weekly payroll.</p>
        <a class="button" href="/manager/payroll-problems/">Review Payroll Problems</a>
        <a class="button secondary" href="/manager/weekly-summary/?week_start=2026-06-15">Weekly Summary</a>
    </div>
